import type { FxDbDriverNS, IDbDriver } from "../../../db-driver/index";
...
import { Database } from "../DB/dm";
import shared = require("./_shared");
import DDL = require("./_ddl-sql");
import { FxSqlQuery, Query } from "../../../sql-query/Query";
import Sync = require("../../../sql-ddl-sync/index");
...
export const Driver: FxOrmDMLDriver.DMLDriverConstructor = function (
    ...
    connection: FxOrmDb.Database<Class_DbConnection>,
...
Driver.prototype.ping = function (
    this: FxOrmDMLDriver.DMLDriver, cb: FxOrmCommon.VoidCallback
) {
    this.db.ping();
...
Driver.prototype.on = function (
    this: FxOrmDMLDriver.DMLDriver, ev: string, cb?: FxOrmCommon.VoidCallback
) {
    if (ev == "error") {
...
Driver.prototype.connect = function (
    this: FxOrmDMLDriver.DMLDriver, cb?: FxOrmCommon.GenericCallback<IDbDriver>
) {
...
Driver.prototype.reconnect = function (
    ...
    (this as any).db = new Database(connOpts);
...
Driver.prototype.getQuery = function (this: FxOrmDMLDriver.DMLDriver): FxSqlQuery.Class_Query {
    return this.query;
};
...
Driver.prototype.find = function (
    this: FxOrmDMLDriver.DMLDriver, selectFields, table, conditions, opts
) {
...
Driver.prototype.count = function (
    this: FxOrmDMLDriver.DMLDriver, table, conditions, opts, cb?
) {
    const { from_tuple, pure_table, alias } = Utilities.parseTableInputForSelect(table);
...
Driver.prototype.insert = function (
    this: FxOrmDMLDriver.DMLDriver, table, data, keyProperties, cb?
) {
    ...
    const syncResponse = Utilities.catchBlocking(() => {
        let info;
        const singleKeyProp = keyProperties && keyProperties.length === 1 ? keyProperties[0] : null;
        const isIdentityKey = !!(singleKeyProp && (singleKeyProp.serial || singleKeyProp.type === 'serial'));
        const explicitIdentity = !!(isIdentityKey && data[singleKeyProp.mapsTo] !== undefined && data[singleKeyProp.mapsTo] !== null);
        if (explicitIdentity) {
            this.execSimpleQuery(`SET IDENTITY_INSERT ${this.query.escapeId(table)} ON`);
            try {
                info = this.execSimpleQuery(q);
            } finally {
                this.execSimpleQuery(`SET IDENTITY_INSERT ${this.query.escapeId(table)} OFF`);
            }
        } else {
            info = this.execSimpleQuery(q);
        }
        ...
        if (keyProperties) {
            if (keyProperties.length == 1) {
                ...
                const isIdentityKey = !!(keyProp.serial || keyProp.type === 'serial');
                if (isIdentityKey) {
                    if (info && info.hasOwnProperty("insertId") && info.insertId !== 0) {
                        ids[keyProp.name] = info.insertId;
                    } else {
                        let identityRes = null;
                        try {
                            identityRes = this.execSimpleQuery('SELECT @@IDENTITY as id');
                        } catch (e) {}
                        if (identityRes && identityRes[0] && identityRes[0].id !== undefined && identityRes[0].id !== null) {
                            ids[keyProp.name] = identityRes[0].id;
                        } else {
                            ids[keyProp.name] = data[keyProp.mapsTo];
                        }
                    }
                } else {
                    ids[keyProp.name] = data[keyProp.mapsTo];
...
Driver.prototype.update = function (
    this: FxOrmDMLDriver.DMLDriver, table, changes, conditions, cb?
) {
...
Driver.prototype.remove = function (
    this: FxOrmDMLDriver.DMLDriver, table, conditions, cb?
) {
...
Driver.prototype.clear = function (
    this: FxOrmDMLDriver.DMLDriver, table, cb?
) {
...
Driver.prototype.valueToProperty = function (
    this: FxOrmDMLDriver.DMLDriver, value, property
) {
    var customType;
