...
describe("hasOne", function() {
    ...
    var setup = function(opts) {
        ...
        return function() {
            ...
            var Hole = db.define("hole", { width: { type: 'integer' } });
            Leaf = db.define("leaf", {
                size: { type: 'integer' },
            ...
            helper.dropSync([Tree, Stalk, Hole, Leaf], function() {
                var tree = Tree.createSync({ type: 'pine' });
                treeId = tree[Tree.id];

                var leaf = Leaf.createSync({ size: 14 });
                leafId = leaf[Leaf.id];
                leaf.setTreeSync(tree);

                var stalk = Stalk.createSync({ length: 20 });
                assert.exist(stalk);
                stalkId = stalk[Stalk.id];

                var hole = Hole.createSync({ width: 3 });
                holeId = hole.id;
            });
        };
    };
    ...
    describe("accessors", function() {
        ...
        it("set should associate another instance", function() {
            var stalk = Stalk.oneSync({ length: 20 });
            assert.exist(stalk);
    ...
    [false, true].forEach(function(af) {
        describe("with autofetch = " + af, function() {
            ...
            describe("associating by parent id", function() {
                ...
                before(function() {
                    tree = Tree.createSync({ type: "cyprus" });
                });

                it("should work when calling Instance.save", function() {
                    var leaf = new Leaf({ size: 4, treeId: tree[Tree.id] });
                    leaf.saveSync();
                ...
                it("should work when specifying parentId in the save call", function() {
                    var leaf = new Leaf({ size: 4 });
                    leaf.saveSync({ treeId: tree[Tree.id] });
                ...
                it("should work when calling Model.create", function() {
                    var leaf = Leaf.createSync({ size: 4, treeId: tree[Tree.id] });
    ...
    describe("if not passing another Model", function() {
        it("should use same model", function(done) {
            ...
            helper.dropSync(Person, function() {
                ...
                child.setParentSync(new Person({ name: "Parent" }));
            });
        });
    });
