...
describe("hasOne", function() {
    ...
    var setup = function() {
        return function() {
            ...
            helper.dropSync([Person, Pet], function() {
                ...
                Person.createSync({ name: "John Doe" });
                Person.createSync({ name: "Jane Doe" });
                Pet.createSync({ name: "Deco" });
                Pet.createSync({ name: "Fido" });
            });
        };
    };
    ...
    describe("reverse", function() {
        ...
        describe(".getAccessor()", function() {
            it("should work", function() {
                var John = Person.find({ name: "John Doe" }).firstSync();
            ...
            describe("Chain", function() {
                before(function() {
                    ...
                    var people = Person.findSync({ name: ["Harold", "Hagar", "Nelly", "Narnia"] });
                    assert.exist(people);
                ...
                it(".remove() should not call hooks", function() {
                    var pet = Pet.oneSync({ name: "Finch" });
                    assert.exist(pet);
                    ...
                    var items = Person.findSync({ name: "Harold" });
        ...
        it("should be able to set an array of people as the owner", function() {
            var owners = Person.findSync({ name: ["John Doe", "Jane Doe"] });

            var Fido = Pet.find({ name: "Fido" }).firstSync();
        ...
        describe("findBy()", function() {
            ...
            before(function() {
                var jane = Person.oneSync({ name: "Jane Doe" });
                var deco = Pet.oneSync({ name: "Deco" });
    ...
    xdescribe("reverse find", function() {
        ...
        xit("should be able to find given an association instance", function(done) {
            common.retry(setup(), function(done) {
                Person.find({ name: "John Doe" }).first(function(err, John) {
                    assert.notExist(err);
                    assert.exist(John);
        ...
        xit("should be able to find given a number of association instances with a single primary key", function(done) {
            common.retry(setup(), function(done) {
                Person.find({ name: "John Doe" }).first(function(err, John) {
                    assert.notExist(err);
                    assert.exist(John);
