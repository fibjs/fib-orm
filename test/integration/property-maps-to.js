...
describe("Property.mapsTo", function() {
    ...
    describe("normal", function() {
        ...
        it("should create", function() {
            var book = Book.createSync({ title: "History of the wheel", pages: 297 });
            assert.exist(book);
        ...
        it("should save new", function() {
            var book = new Book({ title: "Stuff", pages: 33 })
            book = book.saveSync();
        ...
        it("should find", function() {
            var book = Book.oneSync({ title: "History of the wheel" });
            assert.exist(book);
            assert.equal(book.title, "History of the wheel");
        });
        ...
        it("should order", function() {
            Book.createSync({ title: "Zzz", pages: 2 });
            Book.createSync({ title: "Aaa", pages: 3 });
    ...
    describe("keys", function() {
        ...
        it("should create", function() {
            var person = Person.createSync({ firstName: 'John', lastName: 'Smith', age: 48 });
            assert.exist(person);
        ...
        it("should save new", function() {
            var person = new Person({ firstName: 'Jane', lastName: 'Doe', age: 50 });
        ...
        it("should find", function() {
            var person = Person.oneSync({ firstName: 'Jane' });
            assert.exist(person);
            assert.equal(person.firstName, 'Jane');
            assert.equal(person.lastName, 'Doe');
        });
        ...
        it("should count", function() {
            var person = Person.createSync({ firstName: 'Greg', lastName: 'McDoofus', age: 30 });

            var count = Person.find({ firstName: 'Greg', lastName: 'McDoofus' }).countSync();
            assert.equal(count, 1);
        });

        it("should chain delete", function() {
            var person = Person.createSync({ firstName: 'Alfred', lastName: 'McDoogle', age: 50 });

            var count = Person.find({ firstName: 'Alfred', lastName: 'McDoogle' }).countSync();
            assert.equal(count, 1);

            Person.find({ firstName: 'Alfred', lastName: 'McDoogle' }).removeSync();
