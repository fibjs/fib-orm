...
export namespace FxOrmHook {
    ...
    export interface HookActionWithCtxCallback<
        TTHIS = FxOrmInstance.Instance,
    ...
    export interface HookWait<CTX_SELF = FxOrmInstance.Instance, TNEXT_THIS = any>{
        (
            self: CTX_SELF,
            hook: FxOrmCommon.Arraible<
                HookActionCallback | HookActionWithCtxCallback
                | FxOrmCommon.Arraible<HookActionCallback | HookActionWithCtxCallback>
            >,
            nextor: FxOrmCommon.GenericCallback<TNEXT_THIS>,
            ctx?: Record<string, any>
        ): void
    }

    export type HookResultCallback<TTHIS = FxOrmInstance.Instance> = 
        ((this: TTHIS, success?: boolean) => any)
    
    export type HookRetPayloadCallback<TTHIS = FxOrmInstance.Instance, TPAYLOAD = any, TResult = boolean> =
        (this: TTHIS, arg1?: TPAYLOAD, success?: TResult) => any
        
    export type HookRetOnlyPayloadCallback<TTHIS = FxOrmInstance.Instance, TPAYLOAD = any> =
        (this: TTHIS, arg1?: TPAYLOAD) => any
