...
describe('select - postgresql', () => {
  it('select', () => {
    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').build(),
      'select * from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id', 'name').build(),
      'select "id", "name" from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id', 'name').as('label').build(),
      'select "id", "name" as "label" from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id', 'name').select('title').as('label').build(),
      'select "id", "name", "title" as "label" from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id', 'name').as('label').select('title').build(),
    ...
    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select().build(),
      'select * from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select(
    ...
    assert.equal(
      common.Select({ dialect: 'postgresql' }).calculateFoundRows().from('table1').build(),
      'select SQL_CALC_FOUND_ROWS * from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).calculateFoundRows().from('table1').select('id').build(),
      'select SQL_CALC_FOUND_ROWS "id" from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id1', 'name')
    ...
    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').select('id1')
    ...
    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1').count('id')
        .from('table2', 'id2', 'table1', 'id1').count('id').build(),
  ...
  it('alias: valid but useless', () => {
    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1 as t1')
        .build(),
      'select * from "table1"'
    )

    assert.equal(
      common.Select({ dialect: 'postgresql' }).from('table1 t1')
        .build(),
      'select * from "table1"'
    )
  })
  ...
  it('alias: two tables', () => {
    assert.equal(
      common.Select({ dialect: 'postgresql' })
  ...
  describe('select as', () => {
    ...
    beforeEach(() => {
      querySelect = common.Select({ dialect: 'postgresql' });
